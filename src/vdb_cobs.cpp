...
VDBCOBSSerialDevice::VDBCOBSSerialDevice(int32_t port, int32_t baud_rate)
    : port(port), baud_rate(baud_rate), outbound_packets() {

    serial_task = vex::task(VDBCOBSSerialDevice::serial_thread, (void *)this, vex::thread::threadPriorityHigh);
    decode_task = vex::task(VDBCOBSSerialDevice::decode_thread, (void *)this, vex::thread::threadPriorityHigh);
}

void VDBCOBSSerialDevice::handle_inbound_byte(uint8_t b) {
...
int VDBCOBSSerialDevice::decode_thread(void *vself) {

    VDBCOBSSerialDevice &self = *(VDBCOBSSerialDevice *)vself;
    Packet decoded = {};
...
bool VDBCOBSSerialDevice::write_packet_if_avail() {
...
int VDBCOBSSerialDevice::serial_thread(void *vself) {
    VDBCOBSSerialDevice &self = *(VDBCOBSSerialDevice *)vself;
...
bool VDBCOBSSerialDevice::send_cobs_packet(const Packet &pac) {
    outbound_mutex.lock();
...
void VDBCOBSSerialDevice::cobs_encode(const Packet &in, WirePacket &out) {
    out.clear();
    if (in.size() == 0) {
        return;
    }
...
void VDBCOBSSerialDevice::cobs_decode(const WirePacket &in, Packet &out) {
    out.clear();
    if (in.size() == 0) {
        return;
    }
