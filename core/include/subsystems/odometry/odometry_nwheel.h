...
template <int WHEELS> class OdometryNWheel : public OdometryBase {
  public:
    ...
    Pose2d update() override {
        ...
        if (imu != nullptr) {
            ...
            this_angle = -imu->rotation(vex::rotationUnits::rev) * 2 * M_PI;
            // Offset the angle, if we've done a set_position
            this_angle += angle_offset;
        }
    ...
    Pose2d get_position(void) {
        ...
        Pose2d with_wrapped_angle(
          without_wrapped_angle.translation(), without_wrapped_angle.rotation().wrapped_radians_360()
        );
  ...
  private:
    ...
    Pose2d calculate_new_pos(Eigen::Vector<double, WHEELS> radian_deltas, Pose2d old_pose) {
        ...
        if (imu != nullptr) {
            pose_delta(2) = this_angle - old_angle;
        }
        ...
        if (imu != nullptr) {
            new_pose.setRotationDeg(this_angle);
        }
        return new_pose;
    }
